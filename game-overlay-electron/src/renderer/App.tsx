...
// Define the Window interface for Electron access
declare global {
  interface Window {
    electron: {
      toggleClickThrough: (passthrough: boolean) => void;
    };
  }
}
...
function App() {
  ...
  // Toggle click-through mode (Electron-specific)
  const toggleClickThrough = () => {
    const newState = !isClickThrough;
    setIsClickThrough(newState);
    // Call the Electron API exposed in preload
    window.electron.toggleClickThrough(newState);
  };

  return (
    <div className="overlay">
      {/* Overlay controls - Electron specific */}
      <div className="overlay-controls">
        <button 
          onClick={toggleClickThrough} 
          className={`click-through-button ${isClickThrough ? 'active' : ''}`}
        >
          {isClickThrough ? 'Click Through ON' : 'Click Through OFF'}
        </button>
      </div>
      ...
      {/* Add drag handle for the overlay window */}
